model StirlingTypeAlpha_ex01_copy "StirlingTypeAlpha_ex01_copy"
  ...
  inner Modelica.Fluid.System system(energyDynamics = Modelica.Fluid.Types.Dynamics.DynamicFreeInitial, m_flow_start = 1, massDynamics = Modelica.Fluid.Types.Dynamics.DynamicFreeInitial, momentumDynamics = Modelica.Fluid.Types.Dynamics.DynamicFreeInitial) annotation(
    Placement(transformation(origin={-170,160},
extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Machines.SweptVolume hotVolume(redeclare package Medium = engineAir, clearance = 0.05 * Modelica.Constants.pi / 4 * 0.2 ^ 2, nPorts = 1, pistonCrossArea = Modelica.Constants.pi / 4 * 0.2 ^ 2, s(fixed = false), use_HeatTransfer = true, use_portsData = false,T_start=if use_T_start then system.T_start else Medium.temperature_phX(p_start,h_start,X_start))      annotation (
  ...
  Modelica.Fluid.Machines.SweptVolume sweptVolume 
    annotation (Placement(transformation(origin={-311,85},
extent={{-35,-35},{35,35}})));
  equation
...
grid={2,2})),
    ...
    __OpenModelica_simulationFlags(lv = "LOG_STATS", outputFormat = "plt", s = "dassl"),__MWORKS(VisibleVariable={""}));

end StirlingTypeAlpha_ex01_copy;